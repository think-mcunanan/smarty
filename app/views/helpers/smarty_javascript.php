<?php
/**
 * SmartyJavascript Helper class for wrapping JavascriptHelper methods
 *
 ...
 */
class SmartyJavascriptHelper extends Helper {
 ...
 */
    function _register_smarty_functions(&$smarty) {
        $smarty->register_function('javascript', array(&$this, 'javascript'));
    }

/**
 * Smarty wrapper for JavascriptHelper
 *
 ...
 */
    function javascript($params, &$smarty) {
        ...
        if (!class_exists('ReflectionClass')) {
            $smarty->trigger_error("SmartyJavascript: Error - requires php 5.0 or above", E_USER_NOTICE);
            return;
        }
        ...
        if (empty($function_name)) {
            $smarty->trigger_error("SmartyJavascript: missing 'func' parameter", E_USER_NOTICE);
            return;
        }
        ...
        $classReflector = new ReflectionClass($this->Javascript);
            ...
            foreach ($funcParams as $param) {
                ...
                if (isset($processedParams[$paramName])) {
                    ...
                    if ($param->isDefaultValueAvailable()) {
                        ...
                        $smarty->trigger_error("SmartyJavascript: Error ".$paramName." parameter is required for method ".$function_name, E_USER_NOTICE);
                    } else {
        ...
        if (count($arrayParams)) {
            ...
            $smarty->trigger_error("SmartyJavascript: Error " . $classReflector->name . "::" . $function_name . " is not defined", E_USER_NOTICE);
            return;
        }

        if ($show_call) {
            echo '<pre>SmartyJavascript calling $javascript->' . $function_name . ' with these parameters: <br />';
            var_dump($parameters);
            echo '</pre>';
        }

        $result = call_user_func_array(array($this->Javascript,$function_name),$parameters);
