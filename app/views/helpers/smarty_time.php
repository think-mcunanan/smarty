<?php
/**
 * SmartyTime Helper class for wrapping TimeHelper methods
 *
 ...
 */
class SmartyTimeHelper extends Helper {
 ...
 */
    function _register_smarty_functions(&$smarty) {
        $smarty->register_function('time', array(&$this, 'time'));
    }

/**
 * Smarty wrapper for TimeHelper
 *
 ...
 */
    function time($params, &$smarty) {
        ...
        if (!class_exists('ReflectionClass')) {
            $smarty->trigger_error("SmartyTime: Error - requires php 5.0 or above", E_USER_NOTICE);
            return;
        }
        ...
        if (empty($function_name)) {
            $smarty->trigger_error("SmartyTime: missing 'func' parameter", E_USER_NOTICE);
            return;
        }
        ...
        $classReflector = new ReflectionClass($this->Time);
            ...
            foreach ($funcParams as $param) {
                ...
                if (isset($processedParams[$paramName])) {
                    ...
                    if ($param->isDefaultValueAvailable()) {
                        ...
                        $smarty->trigger_error("SmartyTime: Error ".$paramName." parameter is required for method ".$function_name, E_USER_NOTICE);
                    } else {
        ...
        if (count($arrayParams)) {
            ...
            $smarty->trigger_error("SmartyTime: Error " . $classReflector->name . "::" . $function_name . " is not defined", E_USER_NOTICE);
            return;
        }

        if ($show_call) {
            echo '<pre>SmartyTime calling $time->' . $function_name . ' with these parameters: <br />';
            var_dump($parameters);
            echo '</pre>';
        }

        $result = call_user_func_array(array($this->Time,$function_name),$parameters);
