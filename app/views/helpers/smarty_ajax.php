<?php
/**
 * SmartyAjax Helper class for wrapping AjaxHelper methods
 *
 ...
 */
class SmartyAjaxHelper extends Helper {
 ...
 */
    function _register_smarty_functions(&$smarty) {
        $smarty->register_function('ajax', array(&$this, 'ajax'));
    }

/**
 * Smarty wrapper for AjaxHelper
 *
 * @param mixed $params params from Smarty template call
 * @param Smarty $smarty Smarty object
 * @return mixed
 */
    function ajax($params, &$smarty) {
        // sanity check for php version
        if (!class_exists('ReflectionClass')) {
            $smarty->trigger_error("SmartyAjax: Error - requires php 5.0 or above", E_USER_NOTICE);
            return;
        }
        ...
        if (empty($function_name)) {
            $smarty->trigger_error("SmartyAjax: missing 'func' parameter", E_USER_NOTICE);
            return;
        }
        ...
        $classReflector = new ReflectionClass($this->Ajax);
            ...
            foreach ($funcParams as $param) {
                ...
                if (isset($processedParams[$paramName])) {
                    ...
                    if ($param->isDefaultValueAvailable()) {
                        ...
                        $smarty->trigger_error("SmartyAjax: Error ".$paramName." parameter is required for method ".$function_name, E_USER_NOTICE);
                    } else {
        ...
        if (count($arrayParams)) {
            ...
            $smarty->trigger_error("SmartyAjax: Error " . $classReflector->name . "::" . $function_name . " is not defined", E_USER_NOTICE);
            return;
        }

        if ($show_call) {
            echo '<pre>SmartyAjax calling $ajax->' . $function_name . ' with these parameters: <br />';
            var_dump($parameters);
            echo '</pre>';
        }

        $result = call_user_func_array(array($this->Ajax,$function_name),$parameters);
