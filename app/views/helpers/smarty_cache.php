<?php
/**
 * SmartyCache Helper class for wrapping CacheHelper methods
 *
 ...
 */
class SmartyCacheHelper extends Helper {
 ...
 */
    function _register_smarty_functions(&$smarty) {
        $smarty->register_function('cache', array(&$this, 'cache'));
    }

/**
 * Smarty wrapper for CacheHelper
 *
 * @param mixed $params params from Smarty template call
 * @param Smarty $smarty Smarty object
 * @return mixed
 */
    function cache($params, &$smarty) {
        // sanity check for php version
        if (!class_exists('ReflectionClass')) {
            $smarty->trigger_error("SmartyCache: Error - requires php 5.0 or above", E_USER_NOTICE);
            return;
        }
        ...
        if (empty($function_name)) {
            $smarty->trigger_error("SmartyCache: missing 'func' parameter", E_USER_NOTICE);
            return;
        }
        ...
        $classReflector = new ReflectionClass($this->Cache);
            ...
            foreach ($funcParams as $param) {
                ...
                if (isset($processedParams[$paramName])) {
                    ...
                    if ($param->isDefaultValueAvailable()) {
                        ...
                        $smarty->trigger_error("SmartyCache: Error ".$paramName." parameter is required for method ".$function_name, E_USER_NOTICE);
                    } else {
        ...
        if (count($arrayParams)) {
            ...
            $smarty->trigger_error("SmartyCache: Error " . $classReflector->name . "::" . $function_name . " is not defined", E_USER_NOTICE);
            return;
        }

        if ($show_call) {
            echo '<pre>SmartyCache calling $cache->' . $function_name . ' with these parameters: <br />';
            var_dump($parameters);
            echo '</pre>';
        }

        $result = call_user_func_array(array($this->Cache,$function_name),$parameters);
