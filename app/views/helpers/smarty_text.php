<?php
/**
 * SmartyText Helper class for wrapping TextHelper methods
 *
 ...
 */
class SmartyTextHelper extends Helper {
 ...
 */
    function _register_smarty_functions(&$smarty) {
        $smarty->register_function('text', array(&$this, 'text'));
    }

/**
 * Smarty wrapper for TextHelper
 *
 ...
 */
    function text($params, &$smarty) {
        ...
        if (!class_exists('ReflectionClass')) {
            $smarty->trigger_error("SmartyText: Error - requires php 5.0 or above", E_USER_NOTICE);
            return;
        }
        ...
        if (empty($function_name)) {
            $smarty->trigger_error("SmartyText: missing 'func' parameter", E_USER_NOTICE);
            return;
        }
        ...
        $classReflector = new ReflectionClass($this->Text);
            ...
            foreach ($funcParams as $param) {
                ...
                if (isset($processedParams[$paramName])) {
                    ...
                    if ($param->isDefaultValueAvailable()) {
                        ...
                        $smarty->trigger_error("SmartyText: Error ".$paramName." parameter is required for method ".$function_name, E_USER_NOTICE);
                    } else {
        ...
        if (count($arrayParams)) {
            ...
            $smarty->trigger_error("SmartyText: Error " . $classReflector->name . "::" . $function_name . " is not defined", E_USER_NOTICE);
            return;
        }

        if ($show_call) {
            echo '<pre>SmartyText calling $text->' . $function_name . ' with these parameters: <br />';
            var_dump($parameters);
            echo '</pre>';
        }

        $result = call_user_func_array(array($this->Text,$function_name),$parameters);
